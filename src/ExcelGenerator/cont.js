...
async function exportData(req, res) {
  const { date, tipe, month, userId } = req.body;
  ...
  const object = [
    ...
    //7
    {
      query: queries.printDataLaporanPerOrang,
      values: [req.body.userId, utc || null],
      op: "READ_LAPORAN",  
    },
    //8
    {
      query: queries.printDataAbsensiPerOrang,
      values: [req.body.userId, utc || null],
      op: "READ_ABSENSI",  
    },
    //9
    {
      query: queries.printDataSummaryKehadiran,
      values: null,
      op: "READ_ABSENSI",  
    },,
  ];
