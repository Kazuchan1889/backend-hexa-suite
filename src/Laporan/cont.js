...
const postLaporan = async (req, res) => {
  if (check.checkOperation("SELF_FORM", req.userOperation, res)) return;
  const { lokasi, keterangan, jenis, dokumen, tanggal, time, deskripsi } = req.body;
  ...
  pool.query(
    queries.postLaporan,
    [id, date, lokasi, keterangan, jenis, dokumen, tanggal, ontime, time, deskripsi],
    (err, result) => {
      if (err) throw err;
...
const patchLaporanSelf = async(req,res)=>{
  try{
    if (check.checkOperation("SELF_FORM", req.userOperation, res)) return;
    const { 
      lokasi,
    ...
    const result = await pool.query(queries.PatchLaporan,[
      lokasi,
      keterangan,
      jenis,
      dokumen,
...
const getLaporanByTanggal = async(req,res)=>{
  try{
    ...
    const date = req.params.Tgl;

    const result = await pool.query(queries.getLaporanByDate,[date]);
    return res.status(200).send(result.rows);
...
const getAllUserLaporan = async(req,res)=>{
  try{
    if (check.checkOperation("READ_LAPORAN", req.userOperation, res)) return;
    const result = await pool.query(queries.getAllLaporan);
    return res.status(200).send(result.rows);
  }catch(error){
    return res.json(error);
  }
